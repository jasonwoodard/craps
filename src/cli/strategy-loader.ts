...
/**
 * The canonical spec codec lives in `strategy-spec.ts` — a dependency-free
 * module so the server, the CLI, and the Web UI parse specs with one
 * implementation. Re-exported here for back-compat with existing importers.
 */
export {
  parseStrategySpec,
  canonicalizeSpec,
  type StrategySpecOptions,
  type ParsedStrategySpec,
} from './strategy-spec';
